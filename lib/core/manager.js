...
Manager.prototype.resolveSubstitutions = function (dependencies) {
  var substitutions = _.assign(this.substitutions, this.json.substitutions);
  var dependencies_with_substitutions = {};

  //console.log({name: this.name, dependencies: dependencies, substitutions: substitutions});

  _(dependencies).each(function(version, name){
    //console.log('**', name, _.has(substitutions, name));
    if (_.has(substitutions, name)) {
      if (_.isEmpty(substitutions[name]) || !substitutions[name]) {
        // don't include dependency
        template('action', { name: 'substituting', shizzle: name + '#' + version + ' with nothing'}).on('data', this.emit.bind(this, 'data'));
      } else if (_.isString(substitutions[name])) {
        // "name": "semver"
        // todo: log it
        //console.log('Substituting', name, 'with', substitutions[name]);
        template('action', { name: 'substituting', shizzle: name + '#' + version + ' with ' + name + '#' + substitutions[name]}).on('data', this.emit.bind(this, 'data'));
        dependencies_with_substitutions[name] = substitutions[name];
      } else {
        // "name": { "name": "semver" }
        // todo: log it
        //console.log('Substituting', name, 'with', substitutions[name]);
        template('action', { name: 'substituting', shizzle: name + '#' + version + ' with ' + _.map(substitutions[name], function(value, key){ return key + '#' + value;}).join(', ')}).on('data', this.emit.bind(this, 'data'));;
        _.assign(dependencies_with_substitutions, substitutions[name]);
      }
    } else {
      dependencies_with_substitutions[name] = dependencies[name];
    }
  }.bind(this));

  return dependencies_with_substitutions;
};
...
Manager.prototype.resolveFromJson = function () {
  this.once('loadJSON', function () {
    ...
    dependencies = this.resolveSubstitutions(dependencies);
...
Manager.prototype.createJSON = function () {
  ...
  this.substitutions = this.json.substitutions;
};
